...
namex(char *path, int nameiparent, char *name)
{
	...
	if ((curproc != 0) && (curproc->in_newroot == 1)) {
		if (*path == '/') {
			ip = iget(curproc->cwd->dev, curproc->cwd->inum);
		}
		else {
			ip = idup(curproc->cwd);
		}
...
// -------------------------------------------------------------------------------------------------------------
		if (curproc -> in_newroot && ip == curproc->newroot_inode && namecmp(name, "..") == 0){
			 // Prevent escaping the container root
            if (nameiparent && *path == '\0') {
                // Stop here if parent directory is requested
                iunlock(ip);
                return ip;
            }
            next = idup(ip);
...
int
is_inode_directory(struct inode *ip) {
    ilock(ip);
